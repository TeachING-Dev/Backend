...
### 흩어진 정보를 연결해 나만의 지식으로 만드는 AI 학습 서비스

**URL로 저장한 학습 자료(블로그, 유튜브 등)를 AI가 자동 분석·요약하고,<br/>
RAG 기반 챗봇과 개인 맞춤형 학습 경로 '티칭맵'을 통해<br/>
단계별로 학습을 이끌어주는 AI 학습 관리 서비스입니다.**
...
TeachING은 흩어진 온라인 학습 자료를 한곳에 모으고, AI가 튜터처럼 함께 학습을 이끌어주는 개인 학습 관리 서비스입니다.

블로그, 유튜브 등에서 발견한 자료의 URL을 저장하면 AI가 자동으로 본문(또는 영상 스크립트)을 분석해 요약과 상세 해설을 생성하고, 벡터 검색(RAG) 기반으로 색인합니다. 사용자는 저장된 자료들을 폴더와 태그로 자유롭게 정리할 수 있고, 궁금한 점은 채팅으로 질문하면 AI가 내가 저장한 자료를 근거로 답변하며 — 관련 자료가 없을 때는 일반 지식으로 보완해 답합니다.

가장 큰 특징은 **티칭맵**입니다. 폴더 안의 자료들을 순서가 있는 학습 단계로 재구성해, 마치 커리큘럼처럼 하나씩 따라가며 학습할 수 있습니다. 각 단계에서 중요한 부분을 하이라이트하면 사용자가 설정한 성향(다정한/엄격한/응원하는)의 'AI 선생님'이 그 부분을 짚어 설명해주고, 학습 진행률도 함께 추적됩니다.

카카오·구글 소셜 로그인으로 간편하게 시작할 수 있으며, 무료/프리미엄 멤버십을 통해 더 많은 자료 저장과 폴더 생성이 가능합니다.
...
- 블로그, 유튜브 등 **온라인 자료의 URL을 저장**하면 AI가 본문/스크립트를 추출해 **요약·상세 분석** 생성
